...
def generate_eod_snapshot(bot_state, current_date_str, is_post_rebalance=False):
    ...
    if not is_post_rebalance:
        ...
        sorted_holdings = dict(
            sorted(
                portfolio_holdings_summary.items(),
        ...
        while True:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(
                        f"Alpaca API Error on batch (attempt {attempt+1}/"
                        f"{max_retries}): HTTP {response.status_code} - "
                        f"{response.text}"
                    )
                except requests.RequestException as e:
...
def main():
    ...
    for account, symphonies in symphony_data_cache.items():
        for sym in symphonies:
            ...
            if TRIGGER_THRESHOLD_PCT > prob_beating >= TAKE_PROFIT_MC_PCT:
                should_arm = True
            ...
            if prob_beating < TAKE_PROFIT_MC_PCT:
                if (
                    ...
                    print(
                        f"  *** {symphony_name} TP-ARMED (Exceptional Gain: "
                        f"MC Prob {prob_beating:.1f}% < {TAKE_PROFIT_MC_PCT}%) ***"
                    )
